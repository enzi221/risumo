...
Use `<!-- lb:require:NAME -->` or `<!-- lb:require:all:NAME -->` in the definition to insert shared instruction lorebooks.
...
`render()` receives the trigger ID, unmodified payload string, and the renderer options table. Return non-empty HTML. Use `h` for element construction. Prefix every owned CSS class with a module-specific namespace.

The renderer options table contains:

- `chatIndex`: Zero-based chat index.
- `messageIndex`: One-based index of the message in the containing MiniTalk data.

Use `chatIndex` and `messageIndex` together when a control must mutate its source message.
