...
local render = assert(loadfile('lb-minitalk-cocoa/lorebooks/renderer.lua'))()
local toggles = {}
...
test.assertTrue(html:find('class="lb-minitalk-cocoa-media"', 1, true), 'MiniCon uses a media wrapper')
test.assertTrue(html:find('class="lb-minitalk-cocoa-openblock"', 1, true), 'OpenBlock content uses its own wrapper')
...
test.assertTrue(html:find('lb-minitalk-cocoa-avatar lb-minitalk-gradient', 1, true), 'Cocoa avatar reuses the shared gradient')
...
test.assertTrue(html:find('popovertarget="cocoa-test-0"', 1, true), 'Room controls target the supplied unique popover')
