{{#when::lb-stats.thoughts::tis::0}}
Think step-by-step for final data, but keep minimal draft per step.
{{/when}}

Follow this template:

1. Protagonist Check, Linearity Check: Is current narrative someone else's recalling? -> #5
2. Last Status (using latest lb-stats block): Exact time, state, key variables
3. Time Flow Estimation: Dialog count or explicit time skips
4. Iterate Each Field: Changes since last status
5. (Proceed)
