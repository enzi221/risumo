...
{{#when::toggle::lb-news.image}}

## Headline image

Use the first item of `posts[]` as the source for one image-generation prompt. Translate the headline's primary subject and event into a visually complete scene inside `headlineImage`.

### Visual target

Select one visually specific moment that directly depicts the primary subject or event. Choose the focal action, the visible participants, the location, and the surrounding objects before writing tags. Include every visible participant and object required for the action to read clearly inside one frame.

For a subject without a single event, depict a concrete activity, person, object, or location central to the subject.

### Composition

Identify the visible subject or action that carries the image before choosing the camera. Give that subject clear primary visual weight. Block the featured people and action inside a specific location, then choose camera position, shot size, foreground elements, depth, and motivated lighting that reveal the subject.

- Establish foreground, middle ground, and background through people, architecture, terrain, props, or atmospheric elements. Use unequal camera distances, controlled overlap, or foreground occlusion when multiple featured people appear.
- Keep each featured person's identity and role legible. Preserve the acting, receiving, observing, approaching, blocking, or reacting relationship through placement, pose, eye direction, movement, contact, or a shared prop.
- Use prominent props and environmental boundaries to identify the location, guide attention, constrain movement, separate people, or connect people.
- State the physical light source or direction and show which visible surfaces, openings, objects, or people receive or block the light.
- Vary featured people's scale and camera distance. Give every occupied and empty area a compositional role. Use location-specific background elements that support the focal subject.

### Prompt components

Write all `headlineImage` fields in English.

- Write `camera` with exactly one perspective and one framing. Choose a perspective from `from above`, `from behind`, `from below`, `from side`, `high up`, `sideways`, `straight-on`. Choose a framing from `portrait`, `upper body`, `cowboy shot`, `feet out of frame`, `full body`, `wide shot`, `lower body`, `head out of frame`, or `eyes out of frame`.
- Write `cast` as the exact featured person count using number plus `girl`, `girls`, `boy`, or `boys`, such as `1girl` or `1girl, 1boy`. Write `no people` when no featured person appears. Count partially visible featured people. Represent anonymous background population with `crowd` in `scene` instead of counting that population in `cast`.
- Start `scene` with `interior` or `exterior`. Continue with concise comma-separated tags for the image medium, location, time, weather, physical lighting, anonymous population, and prominent props. Describe each prominent prop with visible material or color when useful.
- Add one `characters` entry for every identifiable story participant and every person performing an individually described action. Use `characters[0]` when the image has no featured person. Write each `positive` as comma-separated Danbooru-style tags. Start with `girl` or `boy`, then include the age, hair, eyes, skin or species, body type, attire, expression, camera-relative frame position, and depth position visible in the selected framing. Use `negative` only to preserve explicit negative constraints received through any input source, including character descriptions and user settings. Leave `negative` empty when no explicit negative constraint was received.
- Write `description` as one line of concise, objective natural language. Connect the architecture or terrain, foreground, middle ground, background, openings, boundaries, and prominent props into one spatial layout. Place every featured person in that layout and state the focal action, pose, eye direction, interaction, movement, and lighting relationship that tags cannot express.

Map every featured person in `description` to exactly one `characters` entry. Identify a person on first mention with the shortest visible description that distinguishes that person, then reuse that description. Name both participants in an interaction. Describe anonymous background people collectively.
...
{{#when::{{getglobalvar::toggle_lb-news.sampling}}::is::1}}
